...
import { ConvocatoriaCard } from "./_components/convocatoria-card";
import {
  Pagination,
  PaginationContent,
  PaginationItem,
...
type TipoConvocatoria = "activas" | "anteriores";

export default function ConvocatoriasPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState("");
  const [tipo, setTipo] = useState<TipoConvocatoria>("activas");

  const { data, isLoading } = useQuery(
    publicQueries.convocatorias({ page, limit: ITEMS_PER_PAGE, search: search || undefined, tipo }),
  );
  ...
  const handleTipo = (value: TipoConvocatoria) => {
    setTipo(value);
    setPage(1);
  };
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          {!isLoading && (!data || data.data.length === 0) && (
            <EmptyState
              ...
              title={tipo === "activas" ? "No hay convocatorias activas" : "No hay convocatorias anteriores"}
              description={
                search
                  ? "No se encontraron convocatorias con ese criterio de búsqueda."
                  : tipo === "activas"
                    ? "Aún no se han publicado convocatorias. Vuelve pronto para ver las convocatorias."
                    : "No hay convocatorias finalizadas por el momento."
              }
            />
          )}
