...
export function ResponsablesTab({ convocatoriaId }: ResponsablesTabProps) {
  const { data: me } = useAuth();
  const queryClient = useQueryClient();
  const isAdmin = me?.rol === RolUsuario.ADMINISTRADOR;

  // lista de responsables de la convocatoria
  const { data: responsables, isLoading } = useQuery(
    convocatoriaQueries.responsables(convocatoriaId),
  );

  // usuarios con rol responsable_convocatoria (solo si es admin)
  const { data: usuariosData } = useQuery({
    ...usuarioQueries.list({ rol: RolUsuario.RESPONSABLE_CONVOCATORIA, limit: 100 }),
    enabled: isAdmin,
  });

  // filtrar usuarios que ya son responsables de la convocatoria
  ...
  const addMutation = useMutation({
    mutationFn: (usuarioId: number) =>
      addResponsable(convocatoriaId, { usuarioId }),
    onSuccess: () => {
      toast.success("Responsable asignado correctamente");
      queryClient.invalidateQueries({
        queryKey: convocatoriaQueries.responsables(convocatoriaId).queryKey,
      });
      setSelectedUserId("");
    },
    onError: (error: any) => {
  ...
  const removeMutation = useMutation({
    mutationFn: (userId: number) => removeResponsable(convocatoriaId, userId),
    onSuccess: () => {
      ...
      <CardContent className="space-y-4">
        ...
        {!responsables || responsables.length === 0 ? (
          <EmptyState
            ...
            description="No hay responsables asignados a esta convocatoria."
          />
        ) : (
          <Table>
            <TableHeader>
        ...
        {deleteTarget && (
          <ConfirmDialog
            ...
            description={`Se removerá a "${deleteTarget.nombre}" como responsable de esta convocatoria.`}
