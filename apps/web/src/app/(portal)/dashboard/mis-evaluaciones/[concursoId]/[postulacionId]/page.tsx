...
interface PageProps {
  params: Promise<{ concursoId: string; postulacionId: string }>;
}

export default function CalificacionPage({ params }: PageProps) {
  const { concursoId: cId, postulacionId: pId } = use(params);
  ...
  const { data: detalleData, isLoading: isLoadingDetalle } = useQuery(
    evaluacionQueries.detalle(concursoId, postulacionId),
  );

  // cargar rubrica completa
  const { data: rubrica, isLoading: isLoadingRubrica } = useQuery(
    rubricaQueries.detail(concursoId),
  );
  ...
  const completarMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Calificacion enviada para revision");
      queryClient.invalidateQueries({ queryKey: evaluacionQueries.all() });
      router.push(`/dashboard/mis-evaluaciones/${concursoId}`);
    },
    onError: (err: any) => {
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="flex items-center gap-3 border-b bg-white px-4 py-3 shrink-0">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push(`/dashboard/mis-evaluaciones/${concursoId}`)}
        >
            ...
            <div className="h-full overflow-y-auto p-4">
              ...
              {formulario ? (
                ...
                <p className="text-sm text-secondary-400">
                  No se encontro el formulario del concurso.
                </p>
              )}
            </div>
          </ResizablePanel>
