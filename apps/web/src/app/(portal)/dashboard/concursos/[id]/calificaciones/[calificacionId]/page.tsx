...
export default function RevisionCalificacionPage({ params }: PageProps) {
  ...
  const { data, isLoading: isLoadingCalif } = useQuery(
    calificacionQueries.detalle(concursoId, calificacionId),
  );
  ...
  const { data: archivos, isLoading: isLoadingArchivos } = useQuery({
    ...archivoQueries.list(concursoId, data?.postulacion?.id ?? 0),
  ...
  const aprobarMutation = useMutation({
    mutationFn: () => aprobarCalificacion(concursoId, calificacionId),
    onSuccess: () => {
      toast.success("Calificacion aprobada");
  ...
  const devolverMutation = useMutation({
    mutationFn: () =>
      devolverCalificacion(concursoId, calificacionId, {
        comentarioResponsable: comentarioDevolucion,
      }),
    onSuccess: () => {
      toast.success("Calificacion devuelta al evaluador");
