...
import { createConcurso, updateConcurso } from "@/lib/api/concurso.api";
...
// schema local para el formulario (montoPremio como number, fechas como string date)
// replica createConcursoSchema pero sin el .refine() para usar con react-hook-form
...
const concursoFormSchema = z.object({
  ...
  montoPremio: z.number({ invalid_type_error: "Ingresa un monto valido" }).positive("El monto debe ser mayor a 0"),
...
// transforma ConcursoResponse a valores del formulario
// montoPremio viene como string del backend (Drizzle numeric), lo convertimos a number
function concursoToFormValues(c: ConcursoResponse): ConcursoFormValues {
  return {
    nombre: c.nombre,
    ...
    montoPremio: parseFloat(c.montoPremio),
...
interface ConcursoFormProps {
  initialData?: ConcursoResponse;
}

export function ConcursoForm({ initialData }: ConcursoFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      form.reset(concursoToFormValues(initialData));
    }
  }, [initialData, form]);

  // mutacion crear
  const createMutation = useMutation({
    mutationFn: (dto: CreateConcursoDto) => createConcurso(dto),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      const msg = error.response?.data?.message ?? "Error al crear el concurso";
      toast.error(msg);
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (dto: UpdateConcursoDto) => updateConcurso(initialData!.id, dto),
    onSuccess: () => {
      toast.success("Concurso actualizado correctamente");
      queryClient.invalidateQueries({ queryKey: concursoQueries.all() });
      router.push("/dashboard/concursos");
    },
    onError: (error: any) => {
      const msg = error.response?.data?.message ?? "Error al actualizar el concurso";
      toast.error(msg);
    },
  });
  ...
  function onSubmit(values: ConcursoFormValues) {
    ...
    const dto: CreateConcursoDto = {
      ...
      montoPremio: values.montoPremio,
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Nombre, descripcion y bases del concurso.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Ej: Concurso de Impacto 2026" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      placeholder="Describe brevemente el concurso"
                      rows={3}
                      {...field}
                      value={field.value ?? ""}
                    />
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Bases del concurso</FormLabel>
                  <FormControl>
                    <Textarea
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Periodo de postulacion y anuncio de ganadores.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Inicio de postulacion *</FormLabel>
                  <FormControl>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Cierre de postulacion *</FormLabel>
                  <FormControl>
                    <Input type="date" {...field} value={field.value ?? ""} />
                  </FormControl>
                  <FormMessage />
        ...
        {/* seccion 3: premios y configuracion */}
        <Card>
          <CardHeader>
            <CardTitle>Configuracion</CardTitle>
            <CardDescription>
              Monto asignado, cantidad de ganadores y configuracion del sistema.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4 sm:grid-cols-3">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Numero de ganadores</FormLabel>
                  <FormControl>
                    <Input
                      type="number"
                      min={1}
        ...
        <div className="flex items-center gap-3 justify-end">
          ...
          <Button type="submit" disabled={isLoading} size="lg">
            ...
            {isEditing ? "Guardar cambios" : "Crear concurso"}
          </Button>
        </div>
      </form>
    </Form>
