...
export function EvaluadoresAsignadosTab({
  ...
  const { data: asignaciones, isLoading } = useQuery(
    asignacionQueries.list(concursoId, postulacionId),
  );

  // pool de evaluadores del concurso (para el selector)
  const { data: poolEvaluadores } = useQuery(
    concursoQueries.evaluadores(concursoId),
  );

  // calificaciones del concurso para mostrar estado por evaluador
  const { data: calificaciones } = useQuery(
    calificacionQueries.list(concursoId),
  );
  ...
  const invalidar = () => {
    queryClient.invalidateQueries({
      queryKey: asignacionQueries.list(concursoId, postulacionId).queryKey,
    });
  };
  ...
  const addMutation = useMutation({
    mutationFn: (evaluadorId: number) =>
      assignEvaluadorPostulacion(concursoId, postulacionId, evaluadorId),
    onSuccess: () => {
  ...
  const removeMutation = useMutation({
    mutationFn: (evaluadorId: number) =>
      removeAsignacionEvaluador(concursoId, postulacionId, evaluadorId),
    onSuccess: () => {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Selecciona que evaluadores del concurso calificaran esta postulacion.
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex gap-2">
          <Select value={selectedUserId} onValueChange={setSelectedUserId}>
            <SelectTrigger className="flex-1">
              <SelectValue placeholder="Seleccionar evaluador del concurso..." />
            </SelectTrigger>
            <SelectContent>
              {disponibles.length === 0 ? (
                <SelectItem value="_empty" disabled>
                  {totalPool === 0
                    ? "No hay evaluadores en el concurso"
