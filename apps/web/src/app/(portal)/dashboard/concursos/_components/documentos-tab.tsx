...
export function DocumentosTab({
  ...
  const canEdit = estadoConcurso === EstadoConcurso.BORRADOR;
  ...
  const deleteMutation = useMutation({
    mutationFn: (docId: number) => deleteDocumento(concursoId, docId),
    onSuccess: () => {
      toast.success("Documento eliminado correctamente");
      queryClient.invalidateQueries({
        queryKey: documentoQueries.list(concursoId).queryKey,
      });
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Archivos adjuntos del concurso (bases, reglamentos, anexos).
          </CardDescription>
        </div>
        {canEdit && (
...
function UploadDialog({
  ...
  const uploadMutation = useMutation({
    mutationFn: () => {
      ...
      return uploadDocumento(concursoId, file, nombre.trim(), currentCount + 1);
    },
    onSuccess: () => {
