// Bloque Convocatorias: convocatoria, categoria_convocatoria, responsable_convocatoria, documento_categoria, formulario_dinamico
...
export const propositoDocumento = pgEnum("proposito_documento", ['informativo', 'plantilla', 'jurado'])
...
// categoria dentro de una convocatoria (2 por defecto, N a futuro). Cada categoria tiene
// su propio premio, formulario, rubrica, documentos, evaluadores y postulaciones.
export const categoriaConvocatoria = pgTable("categoria_convocatoria", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "categoria_convocatoria_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	...
	topNSistema: integer("top_n_sistema").default(10).notNull(),
	orden: integer().default(1).notNull(),
	// marca cuando se seleccionaron los ganadores de esta categoria (NULL = no resuelta)
	fechaSeleccionGanadores: timestamp("fecha_seleccion_ganadores", { withTimezone: true, mode: 'string' }),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	foreignKey({
			...
			name: "fk_categoria_convocatoria"
		}).onDelete("cascade"),
	// unique (convocatoria_id, id): destino de la FK compuesta de postulacion y ademas
	// sirve el listado de categorias de una convocatoria (columna lider convocatoria_id).
	unique("uq_categoria_convocatoria_id").on(table.convocatoriaId, table.id),
	check("chk_categoria_monto", sql`monto > (0)::numeric`),
	check("chk_categoria_ganadores", sql`numero_ganadores > 0`),
	check("chk_categoria_top_n", sql`top_n_sistema > 0`),
]);
...
// documentos por categoria, con proposito/audiencia (informativo, plantilla, jurado)
export const documentoCategoria = pgTable("documento_categoria", {
	id: integer().primaryKey().generatedAlwaysAsIdentity({ name: "documento_categoria_id_seq", startWith: 1, increment: 1, minValue: 1, maxValue: 2147483647, cache: 1 }),
	...
	proposito: propositoDocumento().default('informativo').notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_documento_categoria").using("btree", table.categoriaId.asc().nullsLast().op("int4_ops"), table.proposito.asc().nullsLast().op("enum_ops")),
	foreignKey({
			columns: [table.categoriaId],
			foreignColumns: [categoriaConvocatoria.id],
			name: "fk_documento_categoria_categoria"
		}).onDelete("cascade"),
...
export const formularioDinamico = pgTable("formulario_dinamico", {
	...
	foreignKey({
			columns: [table.categoriaId],
			foreignColumns: [categoriaConvocatoria.id],
			name: "fk_formulario_categoria"
		}).onDelete("cascade"),
	unique("uq_formulario_categoria").on(table.categoriaId),
