...
export const preguntaFrecuente = pgTable("pregunta_frecuente", {
	...
	// categoria agrupa preguntas generales; convocatoriaId indica que es especifica de una convocatoria
	...
	foreignKey({
		...
		name: "fk_faq_convocatoria"
	}).onDelete("set null"),
	index("idx_faq_convocatoria_id").using("btree", table.convocatoriaId.asc().nullsLast().op("int4_ops")),
]);
